...
function ch_sd_version() {
    let foot = globalThis.globalThis.gradioApp().getElementById("footer");
    if (!foot) {
        return null;
    }
...
function send_ch_py_msg(msg) {
    ...
    let js_msg_txtbox = globalThis
        .gradioApp()
        .querySelector("#ch_js_msg_txtbox textarea");
...
function get_ch_py_msg() {
    ...
    const py_msg_txtbox = globalThis
        .gradioApp()
        .querySelector("#ch_py_msg_txtbox textarea");
...
globalThis.onUiLoaded(() => {
    ...
    function update_card_for_civitai() {
        ...
        let ch_btn_txts = ["🌐", "💡", "🏷️"];
        let replace_preview_text = globalThis.getTranslation("replace preview");
        ...
        for (const tab_prefix of tab_prefix_list) {
            ...
            //get active extratab
            const active_extra_tab = ((Array.from(
                globalThis
